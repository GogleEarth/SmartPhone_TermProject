...
class ChildrenHouseSearchViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if segue.identifier == "segueToHouseInfoTableView"
        {
            if let navController = segue.destination as? UITableViewController
            {
                ...
                {
                    houseinfoTableViewController.cityname = SearchField.text
